...
def PolImgDisp(ImgPol):
    ImgPol = ImgPol.astype(np.float)
    ImgPol = np.add(np.divide(np.multiply(ImgPol - np.amin(ImgPol), 255. - 0.), np.amax(ImgPol) - np.amin(ImgPol)), 0.).astype(np.uint8)
    return ImgPol

def OverlayCornersOnImg(Img, CornerLabelGaussian, Color=[255,0,0]):
    Img = Img.astype(np.float)
    # Img has to be grayscale
    Corners = np.clip(CornerLabelGaussian.astype(np.float), 0, 255)/255.
    OverlayR = Corners*Color[0]
    OverlayG = Corners*Color[1]
    OverlayB = Corners*Color[2]
    # OpenCV uses BGR color scheme and we are using RGB
    Overlay = np.concatenate((OverlayB[:,:,np.newaxis], OverlayG[:,:,np.newaxis], OverlayR[:,:,np.newaxis]), axis=2)

    if(len(np.shape(Img))<3):
        Img = np.tile(Img[:,:,np.newaxis], (1,1,3))
    elif(len(np.shape(Img))==3):
        # if(np.shape(Img)[2]!=3):
        Img = np.tile(Img[:,:,0], (1,1,3))

    Thld = 0.02
    ImgR = Img[:,:,0]
    ImgR = np.clip(np.clip(OverlayR*(Corners > Thld), 0, 255) + np.clip(ImgR*(Corners <= Thld), 0, 255), 0, 255)
    ImgG = Img[:,:,1]
    ImgG = np.clip(np.clip(OverlayG*(Corners > Thld), 0, 255) + np.clip(ImgG*(Corners <= Thld), 0, 255), 0, 255)
    ImgB = Img[:,:,2]
    ImgB = np.clip(np.clip(OverlayB*(Corners > Thld), 0, 255) + np.clip(ImgB*(Corners <= Thld), 0, 255), 0, 255)
    
    return np.concatenate((ImgB[:,:,np.newaxis], ImgG[:,:,np.newaxis], ImgR[:,:,np.newaxis]), axis=2).astype(np.uint8)
